...
qscreen 专注于持久 shell 会话, 不负责终端布局:

- 每个 qscreen 会话包含一个 shell 和 PTY. qscreen 不创建 pane、平铺布局或终端窗口.
  需要这些功能时, 应使用 `tmux`、GNU `screen` 或其他完整终端 multiplexer.
- Windows 上的 qscreen 通过 ConPTY 运行原生 PowerShell、cmd 或 pwsh 会话, 不依赖
  WSL、Cygwin 或 MSYS2. 同一个 `qscn` CLI 也可在 Linux 和 macOS 上运行.
- 会话可跨 client detach、client 断开和终端窗口关闭继续运行. 会话及 scrollback
  由 daemon 持有, daemon 关闭、daemon 进程终止或机器重启后不会保留.
- `qscreen` 是项目名. `qscn` 是唯一面向用户的可执行文件, 同时包含 client 和按需
  daemon 启动器.
...
1. 打开 [最新 release](https://github.com/dualface/qscreen/releases/latest), 下载对应机器的压缩包:

   | 平台 | Release asset |
   | --- | --- |
   | Windows x86_64 (大多数 PC) | `qscreen-<tag>-windows-x86_64.zip` |
   ...
   | macOS Apple 芯片 | `qscreen-<tag>-macos-arm64.tar.gz` |

2. 解压后在该目录打开终端, 创建 demo 会话:
...
3. 在会话内按下 `Ctrl+B`, 松开后再按 `d`. client 会退出, shell 继续在 daemon 中运行.

4. 可以直接关闭终端窗口. 在解压目录重新打开终端并再次运行 `qscn`. 此时只有一个活动
   会话, 智能命令会自动重新进入:
